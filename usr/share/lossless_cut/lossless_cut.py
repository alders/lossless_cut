...
output = os.popen('arp --numeric')
IP = ''
for line in output.readlines():
    if not "incomplete" in line:
        if not "Address" in line:
            IP = IP + line.split()[0] + ','
...
def send_osd (IP_LIST, TITLE, ORIGIN, DESCRIPTION, EXTRA, IMAGE, PROGRESS_TEXT, PROGRESS, TIMEOUT, STYLE):
...
class Mythtvlosslesscut(object):
    ...
    def __init__(self,
        ...
        try:
            ...
            send_mythnotify("error", "Skrypt lossless_cut", "Wykryto bled w konfiguracji...")
...
Error(%s)\n''') % errmsg)
            ...
            send_mythnotify("error", "Skrypt lossless_cut", "Blad w komunikacji z MythTV...")
...
Error(%s)\n''')
            ...
            send_mythnotify("error", "Skrypt lossless_cut", "Brak niezbednych komponentow do pracy skryptu...")
...
%s\n''') % (MANDITORY))
            ...
            send_mythnotify("error", "Skrypt lossless_cut", "Brak parametru -e lub -m lub -r")
...
%s\n''') % (MANDITORY))
            ...
            send_mythnotify("error", "Skrypt lossless_cut", "Moze byc tylko jeden parametr: -e lub -m lub -r")
...
u' --title "%(mkv_title)s" --attachment-description "%(mkv_description)s"' % \
        ...
        if not self.configuration['rawcutlist'] or \
            ...
            if not 'append_list' in self.configuration:
...
""") % (__title__, __version__, __author__, __purpose__ ))
    ...
    if not os.path.isfile(OPTS.recordedfile):
        ...
        send_mythnotify("error", "Skrypt lossless_cut: Plik", OPTS.recordedfile + " nie istnieje")
...
''') % (common.CONFIG_DIR, common.INIT_CONFIG_FILE, common.CONFIG_FILE))
            ...
            send_mythnotify("error", "Skrypt lossless_cut: nie moge utworzyc:", common.CONFIG_FILE + " w " + common.CONFIG_DIR)
        ...
        try:
            ...
            send_mythnotify("error", "Skrypt lossless_cut", "Wystapil blad zapis pliku konfiguracji")
    ...
    if LOSSLESS_CUT.return_code == 0:
        if OPTS.replace_recorded:
            send_mythnotify("info", title + " " + subtitle, "Zakonczono konwersje do MKV")
        elif OPTS.mythvideo_export:
            send_mythnotify("info", title + " " + subtitle, "Zakonczono eksport do MKV")
        elif OPTS.movepath:
            send_mythnotify("info", title + " " + subtitle, "Zakonczono przenoszenie do MKV")
    else:
        if OPTS.replace_recorded:
            send_mythnotify("error", title + " " + subtitle, "Wystapil blad podczas konwersji do MKV")
        elif OPTS.mythvideo_export:
            send_mythnotify("error", title + " " + subtitle, "Wystapil blad podczas eksportu do MKV")
        elif OPTS.movepath:
            send_mythnotify("error", title + " " + subtitle, "Wystapil blad podczas przenoszenia do MKV")
