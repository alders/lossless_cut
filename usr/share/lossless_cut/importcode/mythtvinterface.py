...
You will need to upgrade your MythTV install to at least a v0.%d+Fixes release.'''),
        ...
        try:
            ...
            #
            self.configuration['mythtv_version'] = u'%d.%d' % (
                    self.OWN_VERSION[0], self.OWN_VERSION[1])
            #
            ## Lossless Cut only supports MythTV v30 and higher
            if self.OWN_VERSION[0] < common.SUPPORTED_VERSIONS:
            ...
            if self.OWN_VERSION[0] >= common.UNSUPPORTED_PRE_VERSION:
...
_(u'''mkvinfo could not find the mkv video's duration, aborting script.''')
        ...
        duration = str(result[1][index:index2])
        # hack to deal with duration has hh.mm.ss.nnnnnnnnn format
        # like "00:47:03.646000000"
        # print ("duraton_full:" + dur)
        duration1, ns = duration.split(".")
        # print ("duration_short:" + duration1)
        timedeltaObj = dt.datetime.strptime(duration1, "%H:%M:%S") - dt.datetime(1900,1,1)
        duration_sec = timedeltaObj.total_seconds()
        # print ("duration_sec:" + str(duration_sec))
        self.new_runtime = int(round(float(duration_sec) / 60))
        self.configuration['new_duration'] = int(duration_sec * 1000)
        #
        return
#
...
file "%s", aborting script.''') % self.configuration['base_name']
        ...
        self.configuration['progstart'] = self.recorded.progstart
        #
        if not 'SQL_starttime' in self.configuration or \
                not 'SQL_progstart' in self.configuration:
            self._set_sql_starttime()
        #
